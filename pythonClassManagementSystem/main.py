...
def classprofile():  # classprofile
    ...
    def cperformance():
        ...
        stuselect1 = ttk.Combobox(cp, values=['-select-', 'PT1', 'PT2', 'HFYEARLY', 'PT3', 'PT4', 'M1', 'M2', 'FINAL'],
    ...
    def sreport():  # student report
        ...
        stuselect1 = ttk.Combobox(ask, values=['-select-', 'PT1', 'PT2', 'HFYEARLY', 'PT3', 'PT4', 'M1', 'M2', 'FINAL'],
    ...
    def mrksheetupload():  # upload marksheet
        ...
        sel = ttk.Combobox(upload, values=['-select-', 'PT1', 'PT2', 'HFYEARLY', 'PT3', 'PT4', 'M1', 'M2', 'FINAL'],
                           state='readonly', textvariable=mex)
        sel.grid(row=2, column=2)
        sel.current(0)
        Label(upload, text='Select Subject').grid(row=3, column=1)
        sel1 = ttk.Combobox(upload, values=['-select-', sub[0], sub[1], sub[2], sub[3], sub[4]], state='readonly',
                            textvariable=subu)
    ...
    Label(cprofile,
          text='**Attendance Records feature will be available on version 1.3 - Stay Connected for More Updates').grid(
        row=10, column=2)
    Label(cprofile, text='**Please do not change the format of csv file(marksheet) as it may get corrupted').grid(
        row=12, column=2)
    ...
    def newclass():  # creating new class
        def ok():
            ...
            if clname == '':
                ...
                ncursor.execute(
                    """CREATE TABLE stdetails (grno INTEGER,studentname TEXT,PT1 INTEGER,PT2 INTEGER,HFYEARLY INTEGER,PT3 INTEGER,PT4 INTEGER,M1 INTEGER,M2 INTEGER,FINAL INTEGER)""")  # table for student details
                ncursor.execute(
                    """CREATE TABLE subdetails (grno INTEGER,studentname TEXT,subject TEXT,PT1 INTEGER,PT2 INTEGER,HFYEARLY INTEGER,PT3 INTEGER,PT4 INTGER,M1 INTEGER,M2 INTEGER,FINAL INTEGER)""")  # table for subject details
                nclass.close()
                newc.destroy()
